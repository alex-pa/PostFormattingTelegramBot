...
def process_part_of_speech(message):
    ...
    post_dict[message.chat.id] = post
    bot.send_message(message.chat.id, word.print_word(), reply_markup=post_markup(message), parse_mode="Markdown")
...
def process_adding_links_context(message):
    ...
    bot.send_message(message.chat.id, post.print_post(), parse_mode="Markdown", reply_markup=ReplyKeyboardRemove())
